...
from ads.models import Author, Ads
...
def ads_listing(request):
    ads_listing = Ads.objects.filter(is_active=True)
...
def ads_category_archive(request, slug):
    ...
    ads_by_category = Ads.objects.filter(category=category, is_active=True).order_by('-is_featured', '-date_created')
...
def ads_state_archive(request, slug):
    ...
    ads_by_state = Ads.objects.filter(state=state,is_active=True)
...
def ads_city_archive(request, slug):
    cities = City.objects.filter(city_name__iexact=slug)
    ads_by_city = Ads.objects.filter(
        Q(city__in=cities) & Q(is_active=True)
    )
...
def ads_author_archive(request, pk):
    ...
    ads_by_author = Ads.objects.filter(author=author, is_active=True)
...
def ads_search(request):
    ...
    if state:
        ...
        ads_search_result = Ads.objects.filter(Q(title__icontains=title) | Q(description__icontains=title), is_active=True).order_by('-is_featured', '-date_created')
    elif category:
